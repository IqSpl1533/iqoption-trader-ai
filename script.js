const API_KEY = "6WYNP43S1RHSZJR7";
...
const grafico = new Chart(ctx, {
  type: 'line',
  data: { labels: [], datasets: [{ label: 'EUR/USD', data: [], borderColor: '#38bdf8', backgroundColor: 'rgba(56,189,248,0.2)', tension: 0.2 }] },
...
async function pegarDados(){
  const url = `https://www.alphavantage.co/query?function=FX_INTRADAY&from_symbol=EUR&to_symbol=USD&interval=1min&outputsize=compact&apikey=${API_KEY}`;
  try{
    const resp = await fetch(url);
    const dados = await resp.json();
    if(!dados["Time Series FX (1min)"]) return null;
    const candles = Object.values(dados["Time Series FX (1min)"]).reverse();
    return candles.map(c=>parseFloat(c["4. close"]));
  }catch(e){
...
setInterval(()=>{
  ...
  if(contador<=0){analisar(); contador=60;}
},1000);
